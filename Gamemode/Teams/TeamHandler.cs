...
namespace FPSMO.Teams
{
    internal static class TeamHandler
    {
        ...
        public static void AddPlayer(Player p, string team = "")
        {
            ...
            {
                red.Add(p);
                FPSMOGame.Instance.MessageMap(CpeMessageType.Normal, String.Format("{0} joined team RED!", p.ColoredName));
            }
            else if (team == "BLUE")
            {
                blue.Add(p);
                FPSMOGame.Instance.MessageMap(CpeMessageType.Normal, String.Format("{0} joined team BLUE!", p.ColoredName));
            }
        }
